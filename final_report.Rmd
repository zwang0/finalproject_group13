...
with the aim for lower variance. By minimizing the shrinkage penalty($\lambda
\Sigma_{j=1}^k |\hat{\beta_j^2}|$ for Ridge and $\lambda \Sigma_{j=1}^k
|\hat{\beta_j}|$ for LASSO) along with RSS, unnecessary estimated coefficients
will be shrunk. The coefficient λ controls the magnitude of the penalty. As it
increases, coefficients of  Ridge tend towards 0 and coefficients of LASSO
shrunk to absolute zero, leading to low variance and low bias. The major
difference between two methods is that LASSO performs selection by removing
variables when coefficients shark to zero.
