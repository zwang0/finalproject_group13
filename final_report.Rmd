...
estimated coefficients will be shrunk. The coefficient $\lambda$ controls the magnitude
of the penalty. As it increases, coefficients of  Ridge tend towards 0 and
coefficients of LASSO shrunk to absolute zero, leading to low variance and low
bias. The major difference between two methods is that LASSO performs selection
by removing variables when coefficients shark to zero.
